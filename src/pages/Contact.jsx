  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen bg-[#06121F] flex items-center justify-center">
        <Card className="max-w-md mx-auto bg-white border border-[#0A1A30]/10 shadow-lg shadow-[#06121F]/5">
          <CardContent className="p-8 text-center">
            <div className="bg-green-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle className="h-8 w-8 text-green-600" />
            </div>
            <h2 className="text-2xl font-bold text-[#0A1A30] mb-4">{t.messageSent}</h2>
            <p className="text-[#334155] mb-6">{t.thankYou}</p>
            <Button 
              onClick={() => setIsSubmitted(false)}
              className="w-full bg-gradient-to-r from-[#D4AF37] to-[#F2D98D] text-[#06121F] font-bold hover:from-[#F2D98D] hover:to-[#D4AF37]"
            >
              {t.sendAnother}
            </Button>
          </CardContent>
      ...
      <section className="bg-[#06121F] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-5xl font-bold text-white mb-6">{t.contactUs}</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {contactMethods.map((method, index) => {
              ...
              return (
                <Card key={index} className="text-center bg-white border border-[#0A1A30]/10 shadow-lg shadow-[#06121F]/5 hover:shadow-xl transition-shadow">
                  <CardContent className="p-8">
                    <div className="bg-[#D4AF37]/15 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                      <Icon className="h-8 w-8 text-[#D4AF37]" />
                    </div>
                    <h3 className="text-xl font-semibold text-[#0A1A30] mb-2">{method.title}</h3>
                    <p className="text-[#334155] mb-4">{method.description}</p>
                    <a 
                      href={method.link}
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select id="propertyType" name="propertyType" value={formData.propertyType} onChange={handleInputChange} className="w-full border border-[#0A1A30]/15 rounded-md px-3 py-2 bg-white text-[#0A1A30]">
                    ...
                    <div>
                      <label htmlFor="subject" className="block text-sm font-medium text-[#334155] mb-2">{t.subject}</label>
                    ...
                    <div>
                      <label htmlFor="message" className="block text-sm font-medium text-[#334155] mb-2">{t.message}</label>
                    ...
                    <Button type="submit" disabled={isSubmitting} className="w-full bg-gradient-to-r from-[#D4AF37] to-[#F2D98D] text-[#06121F] font-bold hover:from-[#F2D98D] hover:to-[#D4AF37] text-lg py-3">
                      {isSubmitting ? (
                        <><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#06121F] mr-2"></div>{t.sending}</> 
                      ) : (
              ...
              <Card className="bg-white border border-[#0A1A30]/10 shadow-lg shadow-[#06121F]/5">
                <CardContent className="p-6">
                  <div className="flex items-center mb-4">
                    ...
                    <h3 className="text-xl font-semibold text-[#0A1A30]">{t.officeHours}</h3>
                  </div>
                  <div className="space-y-2 text-[#334155]">
                    ...
                    <div className="mt-4 p-3 bg-[#D4AF37]/10 border border-[#D4AF37]/30 rounded-lg">
                      <p className="text-sm text-[#0A1A30] font-medium">{t.emergency}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-white border border-[#0A1A30]/10 shadow-lg shadow-[#06121F]/5">
                <CardContent className="p-6">
                  <div className="flex items-center mb-4">
                    ...
                    <h3 className="text-xl font-semibold text-[#0A1A30]">{t.ourLocations}</h3>
                  </div>
                  <div className="space-y-4">
                    {locations.map((location, index) => (
                      <div key={index} className="border-l-4 border-[#D4AF37] pl-4">
                        <h4 className="font-semibold text-[#0A1A30]">{location.name}</h4>
                        <p className="text-[#334155] text-sm">{location.address}</p>
                        <p className="text-[#475569] text-xs mt-1">{location.description}</p>
                      </div>
                    ))}
                  </div>
                </CardContent>
