      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          ...
          <section className="bg-white rounded-xl shadow-lg p-8 my-10">
            ...
            <p className="text-[#334155] mb-6">When hosts rely on PMS API:</p>
            <div className="grid md:grid-cols-2 gap-4">
              {problems.map((problem, index) => (
                <div key={index} className="bg-red-50 rounded-lg p-4 flex items-start gap-3">
                  <XCircle className="w-5 h-5 text-red-600 flex-shrink-0 mt-1" />
                  <span className="text-[#334155]">{problem}</span>
                </div>
              ))}
            </div>
          ...
          <section className="bg-[#0A1A30] border border-[#D4AF37]/20 rounded-xl shadow-lg p-8 my-10">
            <h2 className="text-3xl font-bold text-white mb-6 flex items-center gap-3">
              <Smartphone className="w-8 h-8 text-[#D4AF37]" />
              IPM's API-Free Check-In Framework
            </h2>
            <p className="text-[#C9D2DE] mb-6">Your check-in system should include:</p>

            <div className="space-y-6">
              <div className="bg-[#0F2440] rounded-lg p-6 shadow-md border-l-4 border-[#D4AF37]">
                <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2">
                  <Mail className="w-6 h-6 text-[#D4AF37]" />
                  An automated message from Airbnb
                </h3>
                <p className="text-[#C9D2DE]">
                  <strong className="text-white">Trigger:</strong> "Reservation Confirmed"
                </p>
              </div>
              ...
              <div className="bg-[#0F2440] rounded-lg p-6 shadow-md border-l-4 border-[#D4AF37]">
                ...
                <p className="text-[#C9D2DE] mb-2">Example:</p>
                <code className="bg-[#06121F] px-4 py-2 rounded text-[#D4AF37] font-mono">
                  https://ipm.services/check-in/listing-name
                </code>
              </div>

              <div className="bg-[#0F2440] rounded-lg p-6 shadow-md border-l-4 border-green-500">
                <h3 className="text-xl font-bold text-white mb-3">
                  A Mailchimp (or Typeform/JotForm) embedded form
                </h3>
                <p className="text-[#C9D2DE] mb-3">Collect only:</p>
                <ul className="space-y-2 text-[#C9D2DE]">
                ...
                <p className="text-[#9CA9B8] text-sm mt-3 italic">
                  (Nothing else — Airbnb already gave you the rest)
                </p>
              </div>

              <div className="bg-[#0F2440] rounded-lg p-6 shadow-md border-l-4 border-amber-500">
                <h3 className="text-xl font-bold text-white mb-3">
                  A simple matching key
                </h3>
                <p className="text-[#C9D2DE] mb-3">
                  For guests who struggle with reservation codes, IPM uses:
                </p>
                <div className="space-y-2">
                  <div className="bg-[#06121F] text-[#C9D2DE] rounded p-3">
                    <strong className="text-white">Last Name + Check-In Date</strong>
                  </div>
                  <div className="text-center text-[#9CA9B8] font-bold">OR</div>
                  <div className="bg-[#06121F] text-[#C9D2DE] rounded p-3">
                    <strong className="text-white">Phone Number</strong>
                  </div>
                  <div className="text-center text-[#9CA9B8] font-bold">OR</div>
                  <div className="bg-[#06121F] text-[#C9D2DE] rounded p-3">
                    <strong className="text-white">Reservation Code</strong> (optional)
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0F2440] rounded-lg p-6 shadow-md border-l-4 border-[#D4AF37]">
                ...
                <p className="text-[#C9D2DE] mb-3">Tool options:</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {['Mailchimp', 'Zapier or Make', 'MailParser or Parseur', 'Airtable or Google Sheets'].map((tool) => (
                    <div key={tool} className="bg-[#D4AF37]/10 text-[#C9D2DE] rounded p-3 text-center text-sm font-medium">
                      {tool}
                    </div>
                  ))}
                </div>
                <p className="text-[#C9D2DE] mt-4">
          ...
          <section className="bg-[#0A1A30] border border-[#D4AF37]/20 rounded-xl shadow-lg p-8 my-10">
            ...
            <p className="text-[#C9D2DE] mb-6">Your guests will:</p>
            <div className="space-y-3">
              {guestSteps.map((step, index) => (
                <div key={index} className="bg-[#0F2440] rounded-lg p-4 flex items-center gap-4 shadow-md">
                  <span className="text-3xl">{step.num}</span>
                  <span className="text-lg text-[#C9D2DE]">{step.text}</span>
                </div>
              ))}
            </div>
            <div className="bg-[#06121F] border border-[#D4AF37]/25 text-white rounded-lg p-6 mt-6 text-center">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {systemAdvantages.map((advantage, index) => (
                <div key={index} className="flex items-start gap-3">
                  ...
                  <span className="text-[#334155] font-medium">{advantage}</span>
                </div>
              ))}
            </div>
            <div className="bg-[#D4AF37]/10 border-l-4 border-[#D4AF37] p-6 rounded-r-lg mt-6">
              <p className="text-lg font-bold text-[#0A1A30] m-0">
          ...
          <section className="bg-white rounded-xl shadow-lg p-8 my-10">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-[#334155] font-medium">Use Airbnb automated messages → link to your site</p>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle className="w-6 h-6 text-green-600 flex-shrink-0 mt-1" />
                <p className="text-[#334155] font-medium">Use Mailchimp/Typeform forms → collect guest info</p>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle className="w-6 h-6 text-green-600 flex-shrink-0 mt-1" />
                <p className="text-[#334155] font-medium">Use simple matching keys → no confusion</p>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle className="w-6 h-6 text-green-600 flex-shrink-0 mt-1" />
                <p className="text-[#334155] font-medium">Use Zapier/Make → unify booking + guest data</p>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle className="w-6 h-6 text-green-600 flex-shrink-0 mt-1" />
                <p className="text-[#334155] font-medium">Use IPM → build, install, and optimize the whole system</p>
              </div>
            </div>
          </section>
